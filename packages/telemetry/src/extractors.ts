...
export class SyntheticPoseExtractor implements ITelemetryExtractor {
  readonly landmarkCount = POSE_LANDMARK_COUNT;
  readonly metricKeys = ['squatReps', 'kneeAngle', 'cadence'] as const;
  ...
  extract(_source: HTMLVideoElement | HTMLCanvasElement | ImageBitmap): TelemetryFrame {
    ...
    const phase: 'up' | 'down' = squat > 0.7 ? 'down' : 'up';
    if (this.lastPhase === 'down' && phase === 'up') this.reps++;
 ...
 */
export class BlendshapeTelemetryAdapter implements ITelemetryExtractor {
  readonly landmarkCount = 16;
  readonly metricKeys = ['speakingEnergy', 'jawOpen'] as const;
