...
export class TelemetryPacker {
  static pack(
    ...
    let flags = 0;
    if (metricCount > 0) flags |= 0b01;
    if (quantize) flags |= 0b10;
    ...
    if (quantize) {
      ...
      new Float32Array(buf, offset, landmarkCount * 3).set(
        landmarks.subarray(0, landmarkCount * 3)
      );
  ...
  static unpack(buffer: ArrayBuffer | ArrayBufferView): UnpackedTelemetry | null {
    ...
    if (view.getUint8(2) !== TELEMETRY_VERSION) return null;
    ...
    if (quantized) {
      if (bytes.byteLength < offset + landmarkCount * 3) return null;
      const i8 = new Int8Array(bytes.buffer, bytes.byteOffset + offset, landmarkCount * 3);
      for (let i = 0; i < i8.length; i++) landmarks[i] = i8[i]! / 127;
      offset += landmarkCount * 3;
    } else {
      if (bytes.byteLength < offset + landmarkCount * 12) return null;
      landmarks.set(
        new Float32Array(bytes.buffer, bytes.byteOffset + offset, landmarkCount * 3)
      );
      offset += landmarkCount * 12;
    }

    let metrics: Float32Array | null = null;
    if (metricCount > 0) {
      if (bytes.byteLength < offset + metricCount * 4) return null;
...
export function packMetricsOnly(metrics: Float32Array, seq = 0): ArrayBuffer {
